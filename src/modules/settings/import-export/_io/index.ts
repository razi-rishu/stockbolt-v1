 ...
 * wizard regardless of whether the operator uploaded a CSV or an XLSX.
 * Each module adapter consumes that shape and decides how to validate
 * + transform each row into a DB write.
 *
 * CSV: PapaParse (small, sync, universally compatible).
 * XLSX: SheetJS — dynamic-imported on demand so the ~1 MB library
 *       doesn't bloat the default bundle for operators who only use CSV.
 */
