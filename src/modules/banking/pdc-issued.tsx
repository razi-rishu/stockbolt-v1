  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-ink-primary">{t('banking.pdc_issued_title')}</h1>
          <p className="text-sm text-ink-tertiary mt-1">{t('banking.pdc_issued_desc')}</p>
        </div>
      ...
      <div className="bg-white border border-border-subtle rounded-lg overflow-hidden">
        {isLoading ? (
          ...
          <p className="p-8 text-center text-ink-tertiary">{t('banking.no_pdc_issued')}</p>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead className="bg-surface-muted border-b border-border-subtle">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left font-medium text-ink-secondary">{t('banking.supplier')}</th>
      ...
      <Modal open={showCreateModal} onClose={() => { setShowCreateModal(false); resetForm(); }}
        ...
        <div className="space-y-3 mb-4">
          ...
          <div>
            <label className="block text-xs font-medium text-ink-secondary mb-1">{t('banking.supplier')}</label>
            <SearchableSelect
              options={suppliers.map((s) => ({ value: s.id, label: s.name }))}
              value={contactId}
              onChange={(v) => setContactId(v)}
              placeholder={t('banking.select_supplier')}
              panelWidth={320}
            />
          </div>
          ...
          <div>
            <label className="block text-xs font-medium text-ink-secondary mb-1">{t('banking.payment_account')}</label>
      ...
      <Modal open={!!clearPdcId} onClose={() => setClearPdcId(null)} title={t('banking.clear_pdc_issued')}>
        <div className="space-y-3 mb-4">
          <p className="text-sm text-ink-secondary">{t('banking.clear_pdc_issued_desc')}</p>
          <div>
            <label className="block text-xs font-medium text-ink-secondary mb-1">{t('banking.payment_account')}</label>
