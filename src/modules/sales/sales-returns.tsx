  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">{t('returns.sales_returns_title')}</h1>
          <p className="text-sm text-slate-500 mt-1">{t('returns.sales_returns_desc')}</p>
        </div>
        <Link to="/sales/returns/new">
          <Button variant="primary">{t('returns.new_return')}</Button>
        </Link>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-lg overflow-hidden">
        {isLoading ? (
          ...
          <p className="p-8 text-center text-slate-500">{t('returns.no_sales_returns')}</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  <th className="px-4 py-3 text-left font-medium text-slate-600">{t('returns.return_number')}</th>
                  <th className="px-4 py-3 text-left font-medium text-slate-600">{t('common.date')}</th>
                  <th className="px-4 py-3 text-left font-medium text-slate-600">{t('returns.linked_invoice')}</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {returns.map(sr => (
                  <tr key={sr.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-4 py-3 font-mono text-blue-600">{sr.return_number}</td>
                    <td className="px-4 py-3 text-slate-600">{sr.date}</td>
                    <td className="px-4 py-3 text-slate-500 font-mono text-xs">{sr.invoice_id.slice(0, 8)}…</td>
                    <td className="px-4 py-3 text-slate-600 capitalize">{sr.reason ?? '—'}</td>
                    <td className="px-4 py-3"><StatusBadge status={sr.status} /></td>
                    <td className="px-4 py-3 text-right">
                      <Link to={`/sales/returns/${sr.id}`} className="text-xs text-blue-600 hover:underline">
                        {t('common.view')}
                      </Link>
                    </td>
                  </tr>
