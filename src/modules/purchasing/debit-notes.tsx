  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">{t('returns.debit_notes_title')}</h1>
          <p className="text-sm text-slate-500 mt-1">{t('returns.debit_notes_desc')}</p>
        </div>
        <Link to="/purchasing/debit-notes/new">
          <Button variant="primary">{t('returns.new_debit_note')}</Button>
        </Link>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-lg overflow-hidden">
        {isLoading ? (
          ...
          <p className="p-8 text-center text-slate-500">{t('returns.no_debit_notes')}</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  <th className="px-4 py-3 text-left font-medium text-slate-600">{t('returns.dn_number')}</th>
                  <th className="px-4 py-3 text-left font-medium text-slate-600">{t('common.date')}</th>
                  <th className="px-4 py-3 text-left font-medium text-slate-600">{t('common.supplier')}</th>
                ...
                {notes.map(dn => (
                  <tr key={dn.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-right">
                      <Link to={`/purchasing/debit-notes/${dn.id}`} className="text-xs text-blue-600 hover:underline">
                        {t('common.view')}
                      </Link>
                    </td>
                  </tr>
