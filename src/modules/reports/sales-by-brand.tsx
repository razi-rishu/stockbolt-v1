...
import type { SalesByBrandLine } from '@/data/adapter';
...
export default function SalesByBrandPage() {
  ...
  const [rows, setRows]     = useState<SalesByBrandLine[]>([]);
  const [loading, setLoading] = useState(false);

  const run = async () => {
    if (!company?.id) return;
    setLoading(true);
    try { setRows(await adapter.reports.getSalesByBrand(company.id, from, to)); }
