-- ─────────────────────────────────────────────────────────────────────────
-- StockBolt — edit_vendor_bill v2 (2026-06-13)
-- ─────────────────────────────────────────────────────────────────────────
-- v1 BLOCKED editing a confirmed bill that had triggered a deferred-COGS
-- flush, telling the user to void & recreate. That's wrong for the common
-- case: "I saved the bill, then noticed a missing discount / wrong cost —
-- let me just fix it." Editing must simply replace.
--
-- v2 makes edit fully reversible, including the deferred-COGS flush:
--   1. Reverse the bill JE(s)               (source_type='vendor_bill')
--   2. Reverse the deferred-COGS flush JE(s) (source_type='inventory_cogs')
--   3. Re-queue the flushed deferred-COGS rows → status 'pending'
--      (so re-confirm re-flushes them at the CORRECTED MAC)
--   4. Reverse the bill's stock_ledger rows (un-receive the goods)
--   5. Status → draft
-- Re-confirming the edited bill reposts GL + stock + landed cost + VAT +
-- deferred-COGS flush with the new values — a true "replace".
--
-- Still guarded: payments already applied (unapply/void those first —
-- you can't silently change the amount a payment was settled against).
-- ─────────────────────────────────────────────────────────────────────────
