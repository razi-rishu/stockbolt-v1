-- ─────────────────────────────────────────────────────────────────────────
-- StockBolt v1 — Phase 12 — Migration 13b: fix reset_company_data FK order
-- ─────────────────────────────────────────────────────────────────────────
-- The Phase 12.13 reset_company_data RPC deleted invoice_items in Tier A
-- and deferred_cogs_queue in Tier B. But deferred_cogs_queue has
-- ON DELETE RESTRICT FKs to BOTH invoice_items and invoices — so deleting
-- invoice_items first throws a FK violation, the whole transaction rolls
-- back, and the caller sees "nothing happened" with no GL/data changes.
--
-- Fix: clear deferred_cogs_queue FIRST, before any other deletes. Then
-- the rest of the order works.
-- ─────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- ── Tier 0: deferred_cogs_queue — must clear FIRST because it has
  --    ON DELETE RESTRICT FKs to invoice_items, invoices, AND products.
  --    Deleting any of those first would throw a FK violation and roll
  --    back the entire reset (the bug being fixed in this migration).
  ...
  -- NULL the self-referencing reversal FKs so the bulk DELETE below
  -- doesn't hit ON DELETE RESTRICT within the same statement.
