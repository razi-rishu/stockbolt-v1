-- ============================================================================
-- Phase 63 — Deferred-COGS flush fix + stranded-value repair
--
-- PROBLEM
-- When an item is sold before it is purchased, confirm_invoice defers the COGS
-- (MAC is 0, nothing to cost) and queues a row in deferred_cogs_queue. The
-- flush is supposed to happen when the goods arrive, inside confirm_vendor_bill.
--
-- It did not, because two different definitions of "moving average cost" fight
-- inside the same transaction:
--
--   1. confirm_vendor_bill floors negative stock before averaging
--        v_qty_for_mac := GREATEST(v_old_total_qty, 0)
--      so a receipt covering a backorder correctly yields MAC = the arriving
--      unit cost, and that is what it writes onto the stock_ledger row.
--
--   2. tg_recompute_stock_valuation (phase 29, AFTER INSERT FOR EACH STATEMENT)
--      immediately recomputes the whole partition with an unfloored formula
--        running_avg_cost = CASE WHEN cq <> 0 THEN ROUND(cc/cq,2) ELSE 0 END
--      and when the receipt lands cumulative qty exactly on zero it overwrites
--      that cost with 0.
--
-- The flush loop then re-read running_avg_cost, saw 0, hit
--   IF v_flush_mac <= 0 THEN CONTINUE
-- and left the row 'pending' forever. Nothing re-scans pending rows, so there
-- is no self-heal: the purchase had already debited 1300, the sale never
-- recognised COGS, and stock netted to zero. Inventory overstated and COGS
-- understated by the full purchase value, silently.
--
-- FIX
-- Stop pricing the flush off running_avg_cost. The units received on THIS bill
-- are the ones covering the backorder, so their landed-cost-inclusive unit cost
-- is the correct COGS. The average is unavailable precisely when it is needed
-- and is not the right number anyway.
--
-- tg_recompute_stock_valuation and recompute_stock_valuation are deliberately
-- NOT changed. Their "no stock, no average" rule is defensible and is what E1
-- measures against; touching it would ripple through every valuation surface.
-- This removes the flush's dependency on it instead.
--
...
-- ============================================================================
-- PART 1 — confirm_vendor_bill
--
-- Reproduced verbatim from the live pg_get_functiondef. The ONLY changes are:
--   * four new DECLARE variables (v_arrived_qty, v_arrived_cost, v_consumed,
--     v_used)
--   * the cost lookup at the top of the deferred-COGS flush loop
-- Posting, JE structure, GL legs, MAC math, landed-cost allocation, GRN
-- handling, VAT, round-off and the audit log are all untouched.
-- ============================================================================
...
-- ============================================================================
-- PART 2 — flush_stranded_deferred_cogs
--
-- Repair for rows already stranded by the bug above, and the standing remedy
-- for a pre-existing gap: the flush only ever runs inside confirm_vendor_bill,
-- so stock arriving via GRN, stock adjustment, transfer or opening balance
-- never flushes a deferred row at all.
--
-- Composes post_journal_entry rather than writing general_ledger directly, so
-- balance, period lock, JE numbering and audit all come from the one primitive.
-- One JE per covering bill (post_journal_entry dates the whole entry), with
-- source_type/source_id matching the engine's own flush JE so that a later
-- edit_vendor_bill correctly re-queues these rows.
--
-- Defaults to DRY RUN: returns the proposed lines and posts nothing.
-- ============================================================================
...
REVOKE ALL    ON FUNCTION public.flush_stranded_deferred_cogs(BOOLEAN, DATE) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.flush_stranded_deferred_cogs(BOOLEAN, DATE) TO authenticated;
