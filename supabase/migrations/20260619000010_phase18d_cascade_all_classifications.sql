-- ─────────────────────────────────────────────────────────────────────────
-- StockBolt v1 — Phase 18d: reopen/void reverse advance applications for ALL
--                           receipt classifications (not just advance/on_account)
-- ─────────────────────────────────────────────────────────────────────────
-- BUG: the advance-application cascade only ran when classification was
--   advance/on_account. An 'against_invoice' receipt can leave an unallocated
--   portion in 2400/1400 that is later applied via apply_advance; reopening or
--   voiding it skipped that reversal, orphaning the application and drifting the
--   control balance (the REC-1003 case).
-- FIX: run the cascade for every classification. The per-invoice "multiple
--   advance applications -> raise" guard already protects the ambiguous case,
--   and directly-settled invoices have zero advance_application JEs (no-op).
--   void_payment also gains the reversal_of_id IS NULL filters to match reopen.
-- Reverse-only; no posting-math change. Idempotent (CREATE OR REPLACE).
-- ─────────────────────────────────────────────────────────────────────────
